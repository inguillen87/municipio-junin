...
var MuniAPI = (function() {
  ...
  // 1. WEATHER — wttr.in, no key, real data
  function getWeather() {
    return cached('weather', function() {
      return fetch('https://wttr.in/Junin,BuenosAires,Argentina?format=j1')
        .then(function(r) { return r.json(); })
        .then(function(data) {
          var cc = data.current_condition[0];
          var tomorrow = data.weather && data.weather[1];
          return {
            temp: parseInt(cc.temp_C),
            feelsLike: parseInt(cc.FeelsLikeC),
            humidity: parseInt(cc.humidity),
            wind: parseInt(cc.windspeedKmph),
            windDir: cc.winddir16Point,
            desc: cc.weatherDesc[0].value.trim(),
            code: parseInt(cc.weatherCode),
            uv: parseInt(cc.uvIndex),
            pressure: parseInt(cc.pressure),
            maxTemp: tomorrow ? parseInt(tomorrow.maxtempC) : null,
            minTemp: tomorrow ? parseInt(tomorrow.mintempC) : null,
            ok: true
          };
        });
    }, 10 * 60 * 1000);
