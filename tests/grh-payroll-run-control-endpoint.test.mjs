...
import { createGrhPayrollRunControlHandler } from '../api/grh-payroll-run-control.js';
...
test('endpoint is GET-only, fixed-query, no-store and authenticates before reading', async () => {
  let authCalls = 0;
  let reads = 0;
  const handler = createGrhPayrollRunControlHandler({
  ...
  assert.equal(method.headers['x-municontrol-contract'], 'grh-payroll-run-control-v1');
  ...
  await handler({ method: 'GET', query: { run: 'private' }, headers: {} }, query);
  assert.equal(query.statusCode, 400);
  assert.equal(query.payload.code, 'GRH_PAYROLL_RUN_CONTROL_QUERY_INVALID');
  assert.equal(authCalls, 0);
  assert.equal(reads, 0);
});
...
test('authorized exact-tenant caller reuses workforce-finance read and receives validated aggregates', async () => {
  ...
  const handler = createGrhPayrollRunControlHandler({
    ...
    readArtifactImpl: async options => {
      ...
      return {
        schemaVersion: 'grh-payroll-run-control-v1',
  ...
  assert.deepEqual(calls, [
    ...
    ['artifact', SOURCE_SHA],
    ['contract', 'grh-payroll-run-control-v1'],
  ]);
...
test('missing caller or tenant binding stops before artifact access', async () => {
  let reads = 0;
  const noCaller = createGrhPayrollRunControlHandler({
    requireCapabilityImpl: async () => null,
    readArtifactImpl: async () => { reads += 1; },
  });
  await noCaller({ method: 'GET', query: {}, headers: {} }, responseRecorder());

  const wrongTenant = createGrhPayrollRunControlHandler({
...
test('artifact and contract failures share one detail-free 503 boundary', async () => {
  ...
  for (const handler of [
    createGrhPayrollRunControlHandler({
      ...base,
