...
test('documentation 1.10.0 preserves the governed close, Bot, immutable replay and release truths', () => {
  ...
  assert.equal(routePolicy.ROUTE_POLICY_VERSION, '2026-08-13.11');
  assert.equal(routePolicy.PROTECTED_ROUTES.length, 93);
  assert.deepEqual(runtimeCounts, { serverless: 51, express: 42 });
  ...
  assert.match(inApp, /93\s+(?:rutas|firmas)/i);
  assert.match(inApp, /51\s+Serverless[\s\S]{0,60}42\s+Express/i);
});
...
test('S13 1.10.0 records the exact public release while private evidence stays local', () => {
  ...
  assert.equal(Object.keys(routePolicy.ACTIONS).length, 12);
  assert.equal(Object.keys(routePolicy.PERMISSIONS).length, 53);
  assert.equal(Object.keys(releaseTruth.API_CONTRACTS).length, 18);
  assert.equal(
  ...
  for (const relativePath of releasePaths) {
    ...
    if (relativePath === 'manuales.html') {
      ...
      assert.match(source, /2026-08-13\.11[\s\S]{0,100}2026-08-11\.3/, relativePath);
      assert.match(source, /31\s+recursos[\s\S]{0,80}12\s+acciones[\s\S]{0,80}53\s+permisos[\s\S]{0,80}93\s+(?:rutas|firmas)/i, relativePath);
      assert.match(source, /51 Serverless[\s\S]{0,60}42 Express/i, relativePath);
